...
| OQ-06 | Cycle entropy scope below the whole graph | OPEN — unblocked. Increment 26 established a subsystem concept (`SubsystemLayerPolicies`, a governance-declared id prefix) that OQ-06 **may** reuse, but whether cycle entropy should be scoped that way is its own decision and was deliberately not taken as a side effect: `includeSelfCyclesInCycleEntropy` is still one global boolean, pinned by `cycleEntropyScopeIsStillOneGlobalChoiceSoOq06RemainsUndecided`. Also owns finding (A) below: whether a dimension's relation scope is governable at all. Note SCC detection must not be altered merely to answer this. | 27 |
...
## Findings recorded by increments 25-26, deferred to a named owner
...
| F | **Nested subsystems are unrepresentable.** `SubsystemLayerPolicies` rejects a prefix that would also claim another's nodes, so "everything in `com.foo` except `com.foo.bar`" cannot be declared. Resolving by specificity was rejected as a derivation. | a future decision, if evidence appears |
